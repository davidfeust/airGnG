...
export default function UsersManagerTab() {
    ...
    useEffect(() => {
        ...
        const unsubUsers = onSnapshot(collection(db, 'users'), () => {
            getFromCol('users', setUsersList).then((res) => {
                const filter = res.filter(
                    (item) => item['admin'] === undefined
                );
                setUsersList(filter);
    ...
    const blockChanged = (uid, toBlock) => {
        return Alert.alert(
            'Are your sure?',
            'By pressing yes you confirm to' +
                (!toBlock ? 'un' : '') +
                ' block this user',
            [
                // The "Yes" button
                {
                    ...
                    onPress: async () => {
                        ...
                        await updateDoc(userRef, {
                            ...
                            .catch((error) => {
                                console.log(
                                    'Error blocking / un-blocking user:',
                                    error
                                );
                            });
                    },
    ...
    const searchFilterFunction = (text) => {
        ...
        const newData = arrayHolder.filter(function (item) {
            ...
            const mailText = item.mail
                ? item.mail.toUpperCase()
                : ''.toUpperCase();
    ...
    return (
        <SafeAreaView style={{ marginTop: 10, marginBottom: 65 }}>
            ...
            />
            {usersList && <FlatList data={usersList} renderItem={renderRow} />}
        </SafeAreaView>
    );
}
